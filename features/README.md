...
2. No commit, inclua a marca:
   ...
   ```
   fix: corrige troca de jogador em times completos

   [unlock:domain-game-manager] motivo: bug em switchPlayerFromTeam
   ```
...
```
refactor: ajusta criação de times e regras de pós-partida

[unlock:domain-team-builder] motivo: novo modo de escolha
[unlock:domain-update-draw] motivo: consumir o novo modo
```
...
```powershell
python bin/check-lock.py unlock domain-game-manager
```
...
Desbloqueios registrados:
  ...
  2026-05-20  a1b2c3d  fix: corrige troca de jogador em times completos
  2026-05-22  e4f5g6h  refactor: extrai criacao de times
```
...
- **kebab-case**, slug curto: `domain-game-manager`, `domain-team-builder`, `ui-current-game`.
- Use o vocabulário do domínio (`Pelada`, `Partida`, `Time`, `Jogador`, `Vantagem`).
- Evite nomes genéricos (`servico`, `helper`, `util`) — falham em
  descrever o que está protegido.

### Quando travar

Trave quando:
- A feature foi **testada** (specs passam) e está em uso real.
...
- Configuração que muda demais (`.env`, `*.config.js`, `app.json`).
- Arquivos que dependem fortemente de versão de framework (Expo, RN).
...
python bin/check-lock.py lock domain-game-manager `
    --description "Agregado raiz da pelada" `
    src/domain/Pelada.ts `
    src/domain/Pelada.spec.ts
```
